...
// Qt types are used without namespace prefix for portability
// (IDA SDK Qt uses QT_NAMESPACE=QT which also exposes types at global scope)
QString to_qstring(std::string_view sv) {
    return QString::fromLatin1(sv.data(), static_cast<int>(sv.size()));
}

QString format_ea(uint64_t ea) {
    return QString("0x%1").arg(ea, 0, 16);
}
...
void populate_til_types_impl(QListWidget* list) {
    list->clear();
    ...
    if (list->count() == 0) {
        auto* item = new QListWidgetItem("(No TIL types available)");
        item->setFlags(item->flags() & ~Qt::ItemIsSelectable);
        list->addItem(item);
    }
}

void populate_local_types_impl(QListWidget* list) {
    list->clear();
    ...
    try {
        ...
        if (count == 0) {
            auto* item = new QListWidgetItem("(No local types in database)");
            item->setFlags(item->flags() & ~Qt::ItemIsSelectable);
            list->addItem(item);
            return;
        }
        ...
        for (std::size_t ordinal = 1; ordinal <= count; ++ordinal) {
            try {
                ...
                if (name_result) {
                    auto* item = new QListWidgetItem(to_qstring(*name_result));
                    list->addItem(item);
                }
            } catch (...) {
        ...
        auto* item = new QListWidgetItem("(Unknown error during enumeration)");
        item->setFlags(item->flags() & ~Qt::ItemIsSelectable);
        list->addItem(item);
    }
}
...
void populate_til_types(QListWidget* list) {
    populate_til_types_impl(static_cast<QListWidget*>(list));
}

void populate_local_types(QListWidget* list) {
    populate_local_types_impl(static_cast<QListWidget*>(list));
}
...
void TypeSourceDialog::setup_ui() {
    ...
    auto* label = new QLabel(QString("Select prototype source for call at %1:").arg(format_ea(ea_)));
    layout->addWidget(label);
    layout->addSpacing(12);

    auto* manual_btn = new QPushButton("Enter Manually...");
    manual_btn->setToolTip("Open multi-line prototype editor");
    layout->addWidget(manual_btn);

    auto* standard_btn = new QPushButton("Standard Type  (TIL)");
...
void ManualTypeDialog::setup_ui() {
    ...
    top_layout->addWidget(new QLabel("Input:"));
    ...
    auto* bot_layout = new QVBoxLayout(bot);
    bot_layout->setContentsMargins(0, 0, 0, 0);

    preview_label_ = new QLabel("Preprocessed  (sent to parse_decl):");
    ...
    connect(editor_, &QPlainTextEdit::textChanged, this, &ManualTypeDialog::on_text_changed);
    connect(apply_btn_, &QPushButton::clicked, this, &ManualTypeDialog::on_apply_clicked);
    connect(cancel_btn, &QPushButton::clicked, this, &ManualTypeDialog::on_cancel_clicked);
}
...
void ManualTypeDialog::update_preview() {
    ...
    if (!text.isEmpty() && text.length() < 10000) {
        ...
        try {
            ...
            if (result && !result->empty()) {
                processed = QString::fromLatin1(result->c_str(), static_cast<int>(result->size()));
            }
        } catch (...) {
    ...
    QString trimmed = processed.trimmed();
...
void TilTypeDialog::setup_ui() {
    ...
    connect(search_, &QLineEdit::textChanged, this, &TilTypeDialog::on_search_changed);
    connect(table_, &QTableWidget::cellDoubleClicked, this, &TilTypeDialog::on_type_selected);
    connect(table_, &QTableWidget::itemSelectionChanged, this, &TilTypeDialog::on_selection_changed);
    connect(apply_btn, &QPushButton::clicked, this, &TilTypeDialog::on_apply_clicked);
    connect(cancel_btn, &QPushButton::clicked, this, &TilTypeDialog::on_cancel_clicked);
}
...
void populate_til_types_impl(QTableWidget* table) {
    ...
    if (!tils_result) {
        table->setRowCount(1);
        auto* item = new QTableWidgetItem("(TIL enumeration failed)");
        item->setFlags(item->flags() & ~Qt::ItemIsSelectable);
    ...
    if (entries.empty()) {
        table->setRowCount(1);
        auto* item = new QTableWidgetItem("(No TIL types available)");
        item->setFlags(item->flags() & ~Qt::ItemIsSelectable);
        table->setItem(0, 0, item);
        return;
    }
    ...
    for (std::size_t i = 0; i < entries.size(); ++i) {
        auto* name_item = new QTableWidgetItem(to_qstring(entries[i].first));
        auto* lib_item = new QTableWidgetItem(to_qstring(entries[i].second));
...
void LocalTypeDialog::setup_ui() {
    ...
    connect(search_, &QLineEdit::textChanged, this, &LocalTypeDialog::on_search_changed);
    connect(list_, &QListWidget::itemDoubleClicked, this, &LocalTypeDialog::on_type_selected);
    connect(list_, &QListWidget::itemSelectionChanged, this, &LocalTypeDialog::on_selection_changed);
    connect(apply_btn, &QPushButton::clicked, this, &LocalTypeDialog::on_apply_clicked);
    connect(cancel_btn, &QPushButton::clicked, this, &LocalTypeDialog::on_cancel_clicked);
}
